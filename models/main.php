...
class ConfmgtCkModelMain extends ConfmgtClassModelList
{
	...
	public function __construct($config = array())
	{
		...
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array(
				...
				'title', 'a.title',
				'_theme_name', '_theme_.name',
				'full_paper', 'a.full_paper',
				'camera_ready_paper', 'a.camera_ready_paper',
	...
	*/
	protected function prepareQuery(&$query)
	{
		...
		//WHERE - SEARCH : search_search : search on Title + Key words + User > Name + Theme > Name
		$search_search = $this->getState('search.search');
		$this->addSearch('search', 'a.title', 'like');
		$this->addSearch('search', 'a.key_words', 'like');
		$this->addSearch('search', '_user_.name', 'like');
		$this->addSearch('search', '_theme_.name', 'like');
...
JLoader::register('ConfmgtModelMain', JPATH_SITE_CONFMGT .DS. 'fork' .DS. 'models' .DS. 'main.php');
