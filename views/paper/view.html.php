...
class ConfmgtCkViewPaper extends ConfmgtClassView
{
	...
	*/
	public function display($tpl = null)
	{
		...
		if (in_array($layout, array('paper', 'abstractview', 'fullpaperreview', 'abstractreview')))
		{
	...
	/**
	* Execute and display a template : Abstract review
	*
	* @access	protected
	* @param	string	$tpl	The name of the template file to parse; automatically searches through the template paths.
	*
	...
	* @since	11.1
	*/
	protected function displayAbstractreview($tpl = null)
	{
		$document	= JFactory::getDocument();
		$this->title = JText::_("CONFMGT_LAYOUT_ABSTRACT_REVIEW");
		$document->title = $document->titlePrefix . $this->title . $document->titleSuffix;

		// Initialiase variables.
		$this->model	= $model	= $this->getModel();
		$this->state	= $state	= $this->get('State');
		$state->set('context', 'paper.abstractreview');
		...
		JToolBarHelper::title(JText::_('CONFMGT_LAYOUT_ABSTRACT_REVIEW'), 'confmgt_main');
		// Save & Close
...
JLoader::register('ConfmgtViewPaper', JPATH_SITE_CONFMGT .DS. 'fork' .DS. 'views' .DS. 'paper' .DS. 'view.html.php');
